...
//----------------------------------------------------------------------------------------
// Name: BotGreedyChooseNewTarget()
// Desc: 
//----------------------------------------------------------------------------------------

void BotGreedyChooseNewTarget( size_t PlyrIndex, BotInfo_t *BotInfo )
{
    Backpack_t *Backpack;
    size_t MostWeapsAndItems;
    size_t i;

    // choose a target that has the most weapons and items
    ...
    for( i = 0; i < GetPlayerCount(); i++ )
    {
        ...
        {
            Backpack = GetPlayerBackpack( i );

            if( Backpack->ItemCount > MostWeapsAndItems )
            {
                MostWeapsAndItems = Backpack->ItemCount;
...
//----------------------------------------------------------------------------------------
// Name: BotGreedyReconsiderTarget()
// Desc: 
//----------------------------------------------------------------------------------------

void BotGreedyReconsiderTarget( size_t PlyrIndex, BotInfo_t *BotInfo )
{
    size_t i, Count;
